    ...
    namespace utils {
      namespace iterators {
        ...
        template <class Base, int BitsOut, int BitsIn,
                  class Char = typename boost::iterator_value<
                      Base>::type,  // output character
                  class State = transform_width_state<
                      typename iterator_value<Base>::type, BitsOut, BitsIn> >
        class transform_width_with_state
            : public boost::iterator_adaptor<
                  transform_width_with_state<Base, BitsOut, BitsIn, Char>, Base,
                  Char, single_pass_traversal_tag, Char> {
          friend class boost::iterator_core_access;
          typedef typename boost::iterator_adaptor<
              transform_width_with_state<Base, BitsOut, BitsIn, Char>, Base,
              Char, single_pass_traversal_tag, Char> super_t;

          typedef transform_width_with_state<Base, BitsOut, BitsIn, Char>
              this_t;
          ...
          void increment() {
            ...
            if (m_displacement >= 0 && BitsIn - m_displacement < BitsOut) {
              ...
              if (this->base_reference() != end_) {
                ...
                m_next_buffer = ++this->base_reference() != end_
                                    ? *this->base_reference()
                                    : 0;
                m_nextfull = true;
              }
        ...
        Char
        transform_width_with_state<Base, BitsOut, BitsIn, Char, State>::fill() {
          State &state = this->state();
          if (!state.empty()) {
