  ...
  namespace mime {
    ...
    namespace detail {
      ...
      struct mime_header_parser : qi::grammar<Iterator, Container()> {
        ...
        qi::rule<Iterator, std::string()> token, value, valueCont, valuePart,
            contWS;
      ...
      struct mime_content_type_parser
          : qi::grammar<Iterator, mime_content_type()> {
        mime_content_type_parser()
            : mime_content_type_parser::base_type(content_type_header) {
          content_type_header =
              *qi::lit(' ') >> part >> '/' >> sub_part >> *phrase;
        ...
        qi::rule<Iterator, std::string()> part, sub_part, token, attribute,
            value, quoted_string, extension_token;
      ...
      mime_content_type parse_content_type(const string_type &theHeader) {
        ...
        mime_content_type_parser<typename string_type::const_iterator>
            ct_parser;
      ...
      template <typename string_type>
      static string_type get_ct_value(const string_type &ctString,
                                      const char *key) {
        tracer t(__func__);
      ...
      struct multipart_body_parser : qi::grammar<Iterator, Container()> {
        multipart_body_parser(const std::string &boundary, bool &isMissing)
            : multipart_body_parser::base_type(mimeBody),
              m_is_missing(isMissing) {
          m_is_missing = false;
          //	Thanks to Michael Caisse for the hint to get this working
          mimeBody %= bareSep[phx::ref(m_is_missing) = true] |
                      (+(qi::char_ - sep) >> sep);
      ...
      static void read_multipart_body(
        ...
        innerC mpBody;
        multipart_body_parser<Iterator, innerC> mb_parser(
            separator, mp_body.prolog_is_missing);
        ...
        std::cout << str(boost::format(
                  ...
                  << std::endl;
        std::cout << std::endl << "****** Multipart Body Prolog *******"
                  << std::endl;
        std::copy(mp_body.body_prolog.begin(), mp_body.body_prolog.end(),
                  std::ostream_iterator<char>(std::cout));
        std::cout << std::endl << "****** Multipart Body Epilog *******"
                  << std::endl;
      ...
      static Container read_simplepart_body(Iterator &begin, Iterator end) {
        ...
        std::cout << str(boost::format("Body size %d") % retVal.size())
                  << std::endl;
      ...
      inline void write_boundary(std::ostream &out, std::string boundary,
                                 bool isLast, bool leadingCR = true) {
    ...
    template <class traits = detail::default_types> class basic_mime {
     public:
      ...
      basic_mime(const headerList &theHeaders,
        ...
        constHeaderIter found =
            std::find_if(theHeaders.begin(), theHeaders.end(),
                         detail::find_mime_header<string_type>(
                             detail::k_content_type_header));
      ...
      void set_header_value(const char *key, const string_type &value,
        ...
        if (!replace)
          ...
          if (found == m_headers.end())
            throw std::runtime_error(
                "'header_value' not found - can't replace");
          found->second = value;
        }
      }
      ...
      void append_phrase_to_content_type(const char *key,
        ...
        if (m_headers.end() == found) {
          m_headers.push_back(
              std::make_pair(std::string(detail::k_content_type_header),
                             m_default_content_type));
          found = find_header(detail::k_content_type_header);
        }

        detail::mime_content_type mct =
            detail::parse_content_type(found->second);
      ...
      void set_body(std::istream &in) {
        set_body(std::istream_iterator<char>(in),
      ...
      static boost::shared_ptr<basic_mime<traits> > parse_mime(
        ...
        if (retVal->get_part_kind() == mime_part::simple_part)
          ...
          const char *cont_type =
              boost::iequals(content_type, "multipart/digest")
                  ? "message/rfc822"
                  : "text/plain";

          detail::multipart_body_type<typename traits::body_type>
              body_and_subParts;
          detail::read_multipart_body(begin, end, body_and_subParts,
                                      part_separator);
    ...
    inline std::ostream &operator<<(
        std::ostream &stream, boost::shared_ptr<basic_mime<traits> > part) {
      return stream << *part;
    }
  }
}
