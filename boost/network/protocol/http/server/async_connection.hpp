  ...
  namespace network {
    namespace http {
      ...
      struct async_connection
       ...
       public:
        ...
        template <class Range> void set_headers(Range headers) {
          ...
          if (error_encountered)
            boost::throw_exception(
                boost::system::system_error(*error_encountered));
        ...
        template <class Range> void write(Range const& range) {
          ...
          boost::function<void(boost::system::error_code)> f =
              boost::bind(&async_connection<Tag, Handler>::default_error,
                          async_connection<Tag, Handler>::shared_from_this(),
                          boost::arg<1>());
        ...
        typename disable_if<
            is_base_of<asio::const_buffer, typename Range::value_type>,
            void>::type
       ...
       private:
        typedef boost::array<
            char, BOOST_NETWORK_HTTP_SERVER_CONNECTION_BUFFER_SIZE> buffer_type;

       public:
        typedef iterator_range<buffer_type::const_iterator> input_range;
        typedef boost::function<void(input_range, boost::system::error_code,
                                     std::size_t, connection_ptr)>
        ...
        void read(read_callback_function callback) {
          ...
          if (new_start != read_buffer_.begin()) {
            ...
            thread_pool().post(boost::bind(
                callback, input, boost::system::error_code(),
       ...
       private:
        ...
        typedef boost::array<
            char, BOOST_NETWORK_HTTP_SERVER_CONNECTION_BUFFER_SIZE> array;
        typedef std::list<shared_ptr<array> > array_list;
        typedef boost::shared_ptr<array_list> shared_array_list;
        typedef boost::shared_ptr<std::vector<asio::const_buffer> >
            shared_buffers;
          ...
          if (socket_.is_ssl_enabled() && !handshake_done) {
            ...
            socket_.async_read_some(
                ...
                strand.wrap(boost::bind(
                    ...
                    async_connection<Tag, Handler>::shared_from_this(), state,
          ...
          if (!ec) {
            ...
            switch (state) {
              ...
              case uri:
                input_range = boost::make_iterator_range(new_start, data_end);
                fusion::tie(parsed_ok, result_range) = parser.parse_until(
                    request_parser_type::uri_done, input_range);
                if (!parsed_ok) {
                  client_error();
                  break;
                } else if (parsed_ok == true) {
        ...
        void write_impl(
          ...
          typename boost::range_iterator<Range>::type start =
                                                          boost::begin(range),
                                                      end = boost::end(range);
          while (slice_size != 0) {
