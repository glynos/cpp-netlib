       ...
       private:
          ...
          if (!ec) {
            ...
            if (done) {
              if (request_.method[0] == 'P') {
                ...
                typename std::vector<
                    typename request_header<Tag>::type>::iterator it =
                    std::find_if(request_.headers.begin(),
                ...
                if (content_length != 0) {
                  if (new_start != (buffer_.begin() + bytes_transferred)) {
                    ...
                    content_length -= std::distance(
                        new_start, buffer_.begin() + bytes_transferred);
                  }
                  if (content_length > 0) {
                    socket_.async_read_some(
                        boost::asio::buffer(buffer_),
                        wrapper_.wrap(boost::bind(
                            &sync_connection<
                                 Tag, Handler>::handle_read_body_contents,
                            sync_connection<Tag, Handler>::shared_from_this(),
        ...
        void handle_read_body_contents(boost::system::error_code const &ec,
          ...
          if (!ec) {
            ...
            std::advance(past_end,
                         (std::min)(bytes_to_read, bytes_transferred));
