...
namespace boost { namespace network { namespace http {
    ...
    namespace impl {
        ...
        struct request_headers_wrapper {
            ...
            typename headers_container_type::size_type count(string_type const & key) const {
                return message_.headers().count(key);
            }

            const_iterator begin() const {
                return message_.headers().begin();
            }

            const_iterator end() const {
                return message_.headers().end();
            }
    ...
    template <class Tag>
    inline impl::request_headers_wrapper<Tag>
        headers(basic_request<Tag> const & request_) {
            return impl::request_headers_wrapper<Tag>(request_);
    }

    template <class Tag>
    inline impl::response_headers_wrapper<Tag>
        headers(basic_response<Tag> const & response_) {
