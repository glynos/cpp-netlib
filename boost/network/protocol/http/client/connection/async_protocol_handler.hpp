         ...
         protected:
          ...
          struct to_http_headers {
            ...
            string_type const operator()(U const& pair) const {
              ...
              header_line << pair.first << constants::colon()
                          << constants::space() << pair.second
                          << constants::crlf();
          ...
          logic::tribool parse_status_message(Delegate& delegate_,
                                              Callback callback, size_t bytes) {
            logic::tribool parsed_ok;
            typename buffer_type::const_iterator part_end = part.begin();
            std::advance(part_end, bytes);
            typename boost::iterator_range<typename buffer_type::const_iterator>
          ...
          fusion::tuple<logic::tribool, size_t> parse_headers(
              Delegate& delegate_, Callback callback, size_t bytes) {
            logic::tribool parsed_ok;
            typename buffer_type::const_iterator part_end = part.begin();
            std::advance(part_end, bytes);
            typename boost::iterator_range<typename buffer_type::const_iterator>
...
// the error in debug builds.
          ...
          template <class Delegate, class Callback>
          void parse_body(Delegate& delegate_, Callback callback,
                          size_t bytes) {
