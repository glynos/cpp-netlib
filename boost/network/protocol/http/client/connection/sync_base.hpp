      ...
      namespace impl {
        ...
        struct sync_connection_base_impl {
         protected:
          ...
          void init_socket(Socket& socket_, resolver_type& resolver_,
            ...
            boost::tie(endpoint_iterator, end) =
                resolve_(resolver_, hostname, port);
            while (error && endpoint_iterator != end) {
              socket_.close();
              socket_.connect(
                  tcp::endpoint(endpoint_iterator->endpoint().address(),
          ...
          void read_status(Socket& socket_, basic_response<Tag>& response_,
            ...
            trim_right_if(status_message,
                          boost::is_space() || boost::is_any_of("\r"));
          ...
          void read_body_transfer_chunk_encoding(
            ...
            typename headers_range<basic_response<Tag> >::type
                content_length_range = headers(response_)["Content-Length"];
            if (empty(content_length_range)) {
              typename headers_range<basic_response<Tag> >::type
                  transfer_encoding_range =
                      headers(response_)["Transfer-Encoding"];
              if (empty(transfer_encoding_range)) {
                read_body_normal(socket_, response_, response_buffer,
                                 body_stream);
                return;
              }
                ...
                do {
                  ...
                  if (chunk_size == 0) {
                    ...
                    do {
                      ...
                      std::istreambuf_iterator<char> stream_iterator(
                          &response_buffer);
                      for (; chunk_size > 0 && stream_iterator != eos;
                           --chunk_size)
              ...
              size_t length = lexical_cast<size_t>(
                                  boost::begin(content_length_range)->second) -
                              already_read;
              if (length == 0) return;
              size_t bytes_read = 0;
              while ((bytes_read = boost::asio::read(
                          socket_, response_buffer,
          ...
          void read_body(Socket& socket_, basic_response<Tag>& response_,
            ...
            if (version_major == 1 && version_minor == 0) {
              ...
              if (response_.version() == "HTTP/1.0")
                ...
                read_body_transfer_chunk_encoding(socket_, response_,
                                                  response_buffer, body_stream);
            } else {
        ...
        struct sync_connection_base {
          ...
          new_connection(resolver_type& resolver,
                         resolver_function_type resolve, bool https,
          ...
          virtual void send_request_impl(
              string_type const& method, basic_request<Tag> const& request_,
