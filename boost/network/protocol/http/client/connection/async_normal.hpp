      ...
      namespace impl {
        ...
        struct http_async_connection
            : async_connection_base<Tag, version_major, version_minor>,
              protected http_async_protocol_handler<Tag, version_major,
                                                    version_minor>,
          ...
          typedef typename base::resolver_base::resolver_iterator
              resolver_iterator;
          ...
          typedef typename base::resolver_base::resolve_function
              resolve_function;
          ...
          http_async_connection(resolver_type& resolver,
                                resolve_function resolve, bool follow_redirect,
                                int timeout, connection_delegate_ptr delegate)
              : timeout_(timeout),
          ...
          virtual response start(request const& request,
                                 string_type const& method, bool get_body,
            ...
            if (timeout_ > 0) {
              ...
              timer_.async_wait(request_strand_.wrap(
                  boost::bind(&this_type::handle_timeout,
                              this_type::shared_from_this(), boost::arg<1>())));
            }
            return response_;
          }
         ...
         private:
          ...
          void set_errors(boost::system::error_code const& ec) {
            ...
            this->destination_promise.set_exception(
                boost::copy_exception(error));
          ...
          void handle_resolved(boost::uint16_t port, bool get_body,
            ...
            if (!ec && !boost::empty(endpoint_range)) {
              ...
              asio::ip::tcp::endpoint endpoint(iter->endpoint().address(),
                                               port);
              delegate_->connect(
                  endpoint,
                  request_strand_.wrap(boost::bind(
            ...
            if (is_timedout_) {
              ...
              delegate_->write(command_streambuf,
                               request_strand_.wrap(boost::bind(
                                   ...
                                   this_type::shared_from_this(), get_body,
                                   callback, generator, placeholders::error,
            ...
            if (!is_timedout_ && !ec) {
              ...
              delegate_->read_some(boost::asio::mutable_buffers_1(
                                   ...
                                   request_strand_.wrap(boost::bind(
                                       ...
                                       this_type::shared_from_this(), version,
...
#endif
            if (!is_timedout_ && (!ec || ec == boost::asio::error::eof ||
                                  is_ssl_short_read_error)) {
              ...
              switch (state) {
                ...
                case status:
                  parsed_ok = this->parse_status(
                      delegate_, request_strand_.wrap(boost::bind(
                                     &this_type::handle_received_data,
                                     this_type::shared_from_this(), status,
                                     get_body, callback, placeholders::error,
                                     placeholders::bytes_transferred)),
                      bytes_transferred);
                  if (!parsed_ok || indeterminate(parsed_ok)) return;
                ...
                case headers:
                  ...
                  fusion::tie(parsed_ok, remainder) = this->parse_headers(
                      delegate_, request_strand_.wrap(boost::bind(
                                     &this_type::handle_received_data,
                                     this_type::shared_from_this(), headers,
                                     get_body, callback, placeholders::error,
                                     placeholders::bytes_transferred)),
                      bytes_transferred);
                  ...
                  if (callback) {
                    ...
                    typename protocol_base::buffer_type::const_iterator begin =
                        this->part_begin;
                    typename protocol_base::buffer_type::const_iterator end =
                        begin;
                    ...
                    delegate_->read_some(
                        ...
                        request_strand_.wrap(
                            boost::bind(&this_type::handle_received_data,
                                        this_type::shared_from_this(), body,
                                        get_body, callback, placeholders::error,
                                        placeholders::bytes_transferred)));
                  } else {
                  ...
                  if (ec == boost::asio::error::eof ||
                    ...
                    if (callback) {
                      ...
                      if (this->is_chunk_encoding)
                        this->body_promise.set_value(
                            parse_chunk_encoding(body_string));
                      else
                    ...
                    if (callback) {
                      ...
                      delegate_->read_some(
                          ...
                          request_strand_.wrap(boost::bind(
                              &this_type::handle_received_data,
                              this_type::shared_from_this(), body, get_body,
                              callback, placeholders::error,
                              placeholders::bytes_transferred)));
                    } else {
              ...
              boost::system::system_error error(
                  is_timedout_ ? asio::error::timed_out : ec);
              this->source_promise.set_exception(boost::copy_exception(error));
              this->destination_promise.set_exception(
                  boost::copy_exception(error));
              switch (state) {
                case version:
                  this->version_promise.set_exception(
                      boost::copy_exception(error));
                case status:
                  this->status_promise.set_exception(
                      boost::copy_exception(error));
                case status_message:
                  this->status_message_promise.set_exception(
                      boost::copy_exception(error));
                case headers:
                  this->headers_promise.set_exception(
                      boost::copy_exception(error));
                case body:
                  this->body_promise.set_exception(
                      boost::copy_exception(error));
                  break;
                default:
                  BOOST_ASSERT(false && "Bug, report this to the developers!");
          ...
          string_type parse_chunk_encoding(string_type& body_string) {
            ...
            for (typename string_type::iterator iter = std::search(
                     begin, body_string.end(), crlf.begin(), crlf.end());
                 iter != body_string.end();
                 iter = std::search(begin, body_string.end(), crlf.begin(),
                                    crlf.end())) {
