    ...
    namespace http {
      namespace impl {
        ...
        struct http_sync_connection
            : public virtual sync_connection_base<Tag, version_major,
                                                  version_minor>,
          ...
          typedef http_sync_connection<Tag, version_major, version_minor>
              this_type;
          ...
          http_sync_connection(resolver_type& resolver,
                               resolver_function_type resolve, int timeout)
              : connection_base(),
                timeout_(timeout),
          ...
          void init_socket(string_type const& hostname,
                           string_type const& port) {
            connection_base::init_socket(socket_, resolver_, hostname, port,
                                         resolve_);
          }
              ...
              while (generator(chunk)) {
                std::copy(chunk.begin(), chunk.end(),
                          std::ostreambuf_iterator<typename char_<Tag>::type>(
                              &request_buffer));
                chunk.clear();
                connection_base::send_request_impl(socket_, method,
                                                   request_buffer);
              }
            }
            if (timeout_ > 0) {
          ...
          void read_body(basic_response<Tag>& response_,
            ...
            if (version_major == 1 && version_minor == 1 &&
                ...
                boost::iequals(boost::begin(connection_range)->second,
                               "close")) {
              close_socket();
