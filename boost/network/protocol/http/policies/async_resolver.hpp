    ...
    namespace http {
      namespace policies {
        ...
        struct async_resolver
          ...
          typedef boost::function<
              void(resolver_type &, string_type, boost::uint16_t,
         ...
         protected:
          ...
          void resolve(resolver_type &resolver_, string_type const &host,
            ...
            typename resolver_type::query q(resolver_type::protocol_type::v4(),
                                            host,
                                            lexical_cast<string_type>(port));
            resolver_.async_resolve(
                q, resolver_strand_->wrap(
                       boost::bind(&async_resolver<Tag>::handle_resolve,
          ...
          void handle_resolve(string_type const &host,
            ...
            if (!ec && cache_resolved_) {
              boost::fusion::tie(iter, inserted) = endpoint_cache_.insert(
                  std::make_pair(host, std::make_pair(endpoint_iterator,
                                                      resolver_iterator())));
              once_resolved(ec, iter->second);
            } else {
              once_resolved(
                  ec, std::make_pair(endpoint_iterator, resolver_iterator()));
            }
          }
        };
