       ...
       protected:
        ...
        struct connection_impl {
          typedef function<shared_ptr<connection_impl>(
              resolver_type&, basic_request<Tag> const&,
          ...
          connection_impl(resolver_type& resolver, bool follow_redirect,
                          string_type const& host, string_type const& port,
                          ...
                          optional<string_type> const& private_key_file =
                              optional<string_type>())
              : pimpl(impl::sync_connection_base<
                    Tag, version_major,
          ...
          basic_response<Tag> send_request(
              string_type const& method, basic_request<Tag> request_,
              bool get_body, body_callback_function_type callback,
              body_generator_function_type generator) {
            return send_request_impl(method, request_, get_body, callback,
                                     generator);
          }
            ...
            do {
              ...
              try {
                ...
                if (!retry && e.code() == boost::asio::error::eof) {
                  retry = true;
                  pimpl->init_socket(request_.host(), lexical_cast<string_type>(
                                                          request_.port()));
                  continue;
                }
              ...
              typename headers_range<basic_response<Tag> >::type
                  connection_range = headers(response_)["Connection"];
              if (version_major == 1 && version_minor == 1 &&
                  !empty(connection_range) &&
                  boost::begin(connection_range)->second ==
                      string_type("close")) {
                pimpl->close_socket();
                ...
                if (status >= 300 && status <= 307) {
                  ...
                  if (location_header != boost::end(location_range)) {
                    ...
                    connection_ = get_connection_(
                        resolver_, request_, certificate_filename_,
        ...
        connection_ptr get_connection(
            ...
            optional<string_type> const& private_key_file =
                optional<string_type>()) {
          string_type index = (request_.host() + ':') +
                              lexical_cast<string_type>(request_.port());
          connection_ptr connection_;
          ...
          if (it == host_connections.end()) {
            connection_.reset(new connection_impl(
                ...
                boost::bind(&pooled_connection_policy<Tag, version_major,
                            ...
                            boost::arg<3>()),
                boost::bind(
                    &pooled_connection_policy<Tag, version_major,
