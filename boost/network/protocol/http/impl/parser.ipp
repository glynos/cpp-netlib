  ...
  namespace network {
    namespace http {
          ...
          switch (m_headers_parse_state) {
            ...
            case PARSE_URI_QUERY:
              ...
              if (*m_read_ptr == ' ') {
                ...
                return false;
              } else if (m_query_string.size() >=
                         ParserTraits::QUERY_STRING_MAX) {
                return false;
              } else {
            ...
            case PARSE_HEADER_NAME:
              ...
              if (*m_read_ptr == ':') {
                ...
                m_headers_parse_state = PARSE_SPACE_BEFORE_HEADER_VALUE;
                         ...
                         is_special(*m_read_ptr)) {
                return false;
              } else if (m_header_name.size() >=
                         ParserTraits::HEADER_NAME_MAX) {
                return false;
              } else {
            ...
            case PARSE_HEADER_VALUE:
              ...
              if (*m_read_ptr == '\r') {
                ...
                return false;
              } else if (m_header_value.size() >=
                         ParserTraits::HEADER_VALUE_MAX) {
                return false;
              } else {
          ...
          switch (m_chunked_content_parse_state) {
            ...
            case PARSE_EXPECTING_LF_AFTER_CHUNK_SIZE:
              ...
              if (*m_read_ptr == '\x0A') {
                ...
                m_size_of_current_chunk =
                    strtol(m_chunk_size_str.c_str(), 0, 16);
      ...
      std::size_t basic_parser<Tag, Traits>::consume_content(
        ...
        m_bytes_last_read =
            (http_msg.getContentLength() - content_bytes_to_read);
        m_bytes_total_read += m_bytes_last_read;
      ...
      void basic_parser<Tag, Traits>::finish(basic_request<Tag>& http_request) {
        ...
        if (!m_query_string.empty()) {
          if (!parseURLEncoded(http_request.getQueryParams(),
                               m_query_string.c_str(), m_query_string.size()))
        }
        ...
        std::pair<types::headers::const_iterator,
                  types::headers::const_iterator> cookie_pair =
            http_request.getHeaders().equal_range(types::HEADER_COOKIE);
      ...
      template <typename Tag, typename ParserTraits>
      void basic_parser<Tag, Traits>::finish(
          basic_response<Tag>& http_response) {
      ...
      static bool basic_parser<Tag, Traits>::parse_cookie_header(
        ...
        for (string_type::const_iterator string_iterator =
                 cookie_header.begin();
            ...
            case COOKIE_PARSE_VALUE:
              ...
              if (value_quote_character == '\0') {
                ...
                if (*string_iterator == ';' || *string_iterator == ',') {
                  ...
                  cookie_value.erase();
                  parse_state = COOKIE_PARSE_NAME;
                } else if (*string_iterator == '\'' ||
                           *string_iterator == '"') {
                  if (cookie_value.empty()) {
                    // begin quoted value
                    value_quote_character = *string_iterator;
                  } else if (cookie_value.size() >=
                             ParserTraits::COOKIE_VALUE_MAX) {
                    // max size exceeded
                    return false;
                  } else {
